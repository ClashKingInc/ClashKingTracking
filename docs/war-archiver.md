...
It also continuously consumes checker-produced CWL war-tag jobs from `tracking:cwl:war-tags` in Valkey, sharing the finalization request budget. Workers refill without a batch-completion barrier or five-second backlog sleep. It never discovers tags by scanning group rounds. Already-ended payloads are stored from that first fetch; future CWL wars are scheduled at end time plus 30 minutes. See [CWL handoff](cwl.md) for comparison, deduplication, and retry behavior.
