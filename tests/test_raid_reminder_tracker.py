...
@pytest.mark.asyncio
@freeze_time("2025-02-02 19:00:00", tz_offset=0)  # Sunday 19:00 UTC - 12 hours before the end of the raid
async def test_track_raid_reminders():
    ...
    mock_db_client.reminders.find.return_value.to_list = AsyncMock(
        return_value=[
            {"type": "Clan Capital", "time": "12 hr", "_id": "reminder_1", "clan": "clan_1"},
            {"type": "Clan Capital", "time": "12 hr", "_id": "reminder_2", "clan": "clan_2"},
        ]
    )
    ...
    assert tracker.send_to_kafka.call_count == 2, "Not all reminders were sent to Kafka."
...
async def test_fetch_missing_members():
    ...
    mock_raid_log.members = [
        MagicMock(tag="member_1", attack_count=0, attack_limit=5, bonus_attack_limit=0),
        MagicMock(tag="member_2", attack_count=5, attack_limit=5, bonus_attack_limit=0),
    ]
    mock_get_raid_log = AsyncMock(return_value=mock_raid_log)

    tracker = RaidReminderTracker(config=mock_config, db_client=MagicMock(), kafka_producer=AsyncMock())
    ...
    # Verify the results
    assert "member_1" in missing_members, "member_1 should be in the missing members."
    assert "member_2" not in missing_members, "member_2 should not be in the missing members."
...
async def test_process_reminder():
    ...
    tracker = RaidReminderTracker(config=mock_config, db_client=MagicMock(), kafka_producer=AsyncMock())
    tracker.fetch_missing_members = AsyncMock(return_value={"member_1": "details"})
    tracker.send_to_kafka = AsyncMock()
    ...
    tracker.send_to_kafka.assert_awaited_once_with(reminder, {"member_1": "details"})
...
async def test_send_to_kafka():
    ...
    tracker = RaidReminderTracker(config=MagicMock(), db_client=MagicMock(), kafka_producer=mock_kafka_producer)
