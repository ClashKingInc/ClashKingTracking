...
async def test__track_batch():
    ...
    tracker = Tracking(tracker_type="BOT_CLAN")
    tracker.logger = MagicMock()
    tracker.semaphore = AsyncMock()  # or create a real Semaphore but mock the inside
...
async def test__update_reminder():
    ...
    tracker.db_client.active_reminders = MagicMock(find_one_and_update=mock_find_one_and_update)
...
async def test__schedule_reminders_no_reminders():
    ...
    tracker.db_client.reminders = MagicMock(find=MagicMock(return_value=mock_find))

    clan_tag = "#CLANTAG"
    war_tag = "some_war_tag"
    reminder_data = {"end_time": pend.now("UTC").int_timestamp + 3600}
    await tracker._schedule_reminders(clan_tag, war_tag, reminder_data, "war_reminder")
...
async def test__schedule_reminders_with_data():
    ...
    reminder_docs = [
        {"clan": "#CLANTAG", "type": "war_reminder", "time": "12 hr", "_id": ObjectId(), "server": 123},
        {"clan": "#CLANTAG", "type": "war_reminder", "time": "24 hr", "_id": ObjectId(), "server": 123},
    ]
...
def test_is_raids():
    ...
    with patch("pendulum.now", return_value=pend.datetime(2025, 1, 10, 6, 59, tz="UTC")):
    ...
    with patch("pendulum.now", return_value=pend.datetime(2025, 1, 10, 7, 0, tz="UTC")):
...
async def test_run_single_iteration():
    ...
    tracker.logger = MagicMock()
    with patch.object(tracker, "run", side_effect=[None, asyncio.CancelledError("Stop loop")]):
        # Just call run once
        try:
            await tracker.run(None)  # The signature is run(tracker_class, loop_interval, etc.)
