...
async def test_fetch_reminders():
    ...
    tracker = InactivityReminderTracker(config=MagicMock(), db_client=mock_db_client, kafka_producer=AsyncMock())
    reminders = await tracker.fetch_reminders()

    assert reminders == [{"type": "inactivity", "clan": "clan_tag"}]
    mock_db_client.reminders.find.assert_called_once_with({'type': 'inactivity'})
...
async def test_fetch_inactive_members():
    ...
    mock_db_client.player_stats.find.return_value.to_list = AsyncMock(
        return_value=[{"name": "Alice", "last_online": now.int_timestamp - 7200}]
    )
    ...
    assert inactive_members == [{"name": "Alice", "last_online": now.int_timestamp - 7200}]
...
async def test_send_to_kafka():
    ...
    mock_member = {"name": "Alice", "last_online": 1670000000, "_id": "member_1"}
    reminder = {"type": "inactivity", "clan": "clan_tag"}

    tracker = InactivityReminderTracker(config=MagicMock(), db_client=MagicMock(), kafka_producer=mock_kafka_producer)
...
async def test_process_clan():
    ...
    mock_db_client = MagicMock()
    mock_db_client.player_stats.find.return_value.to_list = AsyncMock(
        return_value=[{"name": "Alice", "last_online": 1670000000, "_id": "member_1"}]
    )

    # Initialize the tracker with mocked dependencies
    tracker = InactivityReminderTracker(
        config=mock_config, db_client=mock_db_client, kafka_producer=AsyncMock()
    )
    ...
    tracker.send_to_kafka.assert_awaited_once_with(
        [{"name": "Alice", "last_online": 1670000000, "_id": "member_1"}], reminder
    )
...
async def test_run():
    ...
    mock_db_client = MagicMock()
    mock_db_client.reminders.find.return_value.to_list = AsyncMock(
        return_value=[{"type": "inactivity", "time": "12 hr", "clan": "clan_tag"}]
    )
