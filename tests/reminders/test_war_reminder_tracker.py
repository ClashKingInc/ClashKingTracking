...
async def test_send_to_kafka():
    ...
    mock_db_client = MagicMock()
    mock_db_client.active_reminders.delete_one = (
        AsyncMock()
    )  # Make delete_one an AsyncMock
...
async def test_process_single_reminder():
    ...
    tracker._fetch_reminder_details = AsyncMock(
        return_value={'extra': 'details'}
    )
    ...
    tracker.send_to_kafka.assert_awaited_once_with(
        {
            'job_id': 'job_1',
            'reminder_id': str(fixed_object_id),
            'extra': 'details',
        }
    )
