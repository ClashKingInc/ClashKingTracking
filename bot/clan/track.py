...
from .utils import clan_track, clan_war_track, raid_weekend_track
...
async def main():
    ...
    coc_client = coc.Client(
        raw_attribute=True, key_count=10, throttle_limit=100
    )
    ...
    while True:
        ...
        for clan_tag in clan_tags:
            tasks.append(
                clan_track(
                    clan_tag=clan_tag, coc_client=coc_client, producer=producer
                )
            )
        await asyncio.gather(*tasks)
