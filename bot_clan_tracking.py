...
if __name__ == "__main__":
    ...
    connection_pool = redis.connection.ConnectionPool(max_connections=config.redis_max_connections)
    redis_host = redis.Redis(host=config.redis_ip, port=6379, db=5, password=config.redis_pw, decode_responses=False, connection_pool=connection_pool)
    keys = create_keys([config.coc_email.format(x=x) for x in range(config.min_coc_email, config.max_coc_email + 1)], [config.coc_password] * config.max_coc_email)
