...
func run() error {
	_ = godotenv.Load()
	daysFlag := flag.String("days", "", "comma-separated completed UTC day labels")
	flag.Parse()
	...
	writes, err := scripts.RebuildLegendCloseouts(context.Background(), dsn, days)
	if err != nil {
		return err
	}
	fmt.Printf("rebuilt %d Legend days with %d writes\n", len(days), writes)
	return nil
}
...
func timescaleURLFromEnvironment() string {
	...
	if host == "" {
		dsn := strings.TrimSpace(os.Getenv("TIMESCALE_URL"))
		return dsn
	}
	parsed := &url.URL{
		Scheme: "postgres",
