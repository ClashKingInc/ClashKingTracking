...
async def calculate_player_capital_looted_leaderboards(
    db_client: MongoDatabase,
):
    weekend = (
        pend.parse(gen_raid_date(), strict=False)
        .replace(hour=7)
        .subtract(days=7)
    )
    ...
    for league in ['All'] + leagues:
        ...
        results = await db_client.raid_weekends.aggregate(
            pipeline=pipeline
        ).to_list(length=None)
        ...
        await db_client.player_capital_lb.insert_many(
            documents=results, ordered=False
        )
    ...
    for w in ['2024-05-10', '2024-05-17']:
        ...
        for sort_type in [
            ...
            for league in ['All'] + leagues:
                ...
                clans = await db_client.global_clans.find(
                    {'tag': {'$in': [x.get('tag') for x in results]}},
                    {'name': 1, 'tag': 1},
                ).to_list(length=None)
                ...
                await db_client.clan_capital_lb.insert_many(
                    documents=results, ordered=False
                )
...
async def calculate_raid_medal_leaderboards(db_client: MongoDatabase):
    weekend = (
        pend.parse('2024-05-10', strict=False).replace(hour=7).subtract(days=7)
    )
        ...
        pipeline = [
            ...
            {'$match': condition},
            {
                '$addFields': {
                    'medals': {
                        '$add': [
                            {'$multiply': ['$data.offensiveReward', 6]},
