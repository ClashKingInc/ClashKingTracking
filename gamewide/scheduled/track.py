...
coc_client = coc.Client(
    key_count=10, throttle_limit=500, cache_max_size=0, raw_attribute=True
)
...
async def store_cwl_groups():
    ...
    all_tags = [
        x['_id']
        for x in (
            await db_client.basic_clan.aggregate(pipeline).to_list(length=None)
        )
    ]
    ...
    done_for_this_season = [
        x['_id']
        for x in (
            await db_client.cwl_group.aggregate(pipeline).to_list(length=None)
        )
    ]
...
async def store_cwl_wars():
    ...
    tags_already_found = set(
        [
            x['_id']
            for x in (
                await db_client.clan_wars.aggregate(pipeline).to_list(
                    length=None
                )
            )
        ]
    ...
    for count, tag_group in enumerate(all_tags, 1):
        ...
        for tag in tag_group:
            tasks.append(
                fetch(
                    f"https://api.clashofclans.com/v1/clanwarleagues/wars/{tag.replace('#', '%23')}",
                    session,
                    {'Authorization': f'Bearer {next(coc_client.http.keys)}'},
                    tag,
                )
        ...
        if add_war:
            await db_client.clan_wars.insert_many(
...
async def store_all_leaderboards():
    ...
    for database, function in zip(
        [
            db_client.clan_trophies,
            db_client.clan_versus_trophies,
            db_client.capital,
            db_client.player_trophies,
            db_client.player_versus_trophies,
        ],
        [
            coc_client.get_location_clans,
            coc_client.get_location_clans_builder_base,
            coc_client.get_location_clans_capital,
            coc_client.get_location_players,
            coc_client.get_location_players_builder_base,
        ],
    ):
        ...
        while after is not None:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    f'https://api.clashofclans.com/v1/leagues/29000022/seasons/{year}?limit=100000{after}',
                    headers=headers,
                ) as response:
            ...
            results = await db_client.legend_history.bulk_write(
...
async def update_autocomplete():
    ...
    pipeline = [
        {'$match': {'last_updated': {'$gte': one_day_ago}}},
        {'$project': {'tag': '$tag'}},
        {'$unset': '_id'},
    ]
    all_player_tags = [
        x['tag']
        for x in (
            await db_client.player_stats.aggregate(pipeline).to_list(
                length=None
            )
        )
    ]
...
async def update_region_leaderboards():
    await db_client.region_leaderboard.update_many(
    ...
    for location in locations:
        task = asyncio.ensure_future(
            coc_client.get_location_players(location_id=location)
        )
        tasks.append(task)
    responses = await asyncio.gather(*tasks, return_exceptions=True)
    ...
    for location in locations:
        task = asyncio.ensure_future(
            coc_client.get_location_players_builder_base(location_id=location)
        )
        tasks.append(task)
    responses = await asyncio.gather(*tasks, return_exceptions=True)
...
async def main():
    ...
    scheduler.add_job(
        store_cwl_groups, 'cron', day='9-12', hour='*', minute=35
    )
