...
async def broadcast():
    ...
    coc_client = coc.Client(
        key_count=10, throttle_limit=25, cache_max_size=0, raw_attribute=True
    )
    ...
    for tag_group in all_tags:
        ...
        for response in responses:
            ...
            for item in warlog.get('items', []):
                ...
                item['timeStamp'] = t
                if (right_now - t <= 2592000) and item['opponent'][
                    'tag'
                ] not in clan_tags_alr_found:
                    tags_to_add.append(
                        InsertOne({'tag': item['opponent']['tag']})
                    )

        if tags_to_add:
            try:
                results = await db_client.global_clans.bulk_write(
