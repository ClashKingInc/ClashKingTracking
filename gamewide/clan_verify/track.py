...
from utility.utils import gen_raid_date, gen_season_date
...
async def main():
    ...
    while True:
        try:
            ...
            await db_client.global_clans.aggregate(ranking_pipeline).to_list(
                length=None
            )
            ...
            for tag_group in all_tags:
                try:
                    ...
                    connector = TCPConnector(
                        limit=500, enable_cleanup_closed=True
                    )
                    ...
                    async with ClientSession(
                        ...
                        for tag in tag_group:
                            ...
                            tasks.append(
                                fetch(
                                    f"https://api.clashofclans.com/v1/clans/{tag.replace('#', '%23')}",
                                    session,
                    ...
                    clan_group_members = await db_client.global_clans.find(
                        {'tag': {'$in': tag_group}},
                        {'tag': 1, '_id': 0, 'memberList': 1},
                    ).to_list(length=None)
                    clan_group_members = {
                        x.get('tag'): x.get('memberList', [])
                        for x in clan_group_members
                    }
                    for response in responses:   # type: bytes
                        ...
                        if clan.members == 0:
                            await db_client.deleted_clans.insert_one(
                            ...
                            if clan.tag in clan_group_members:
                                clan_member_list = [
                                    ...
                                    for m in clan_group_members.get(clan.tag)
                                ]
                                new_joins = [
                                    player
                                    for player in clan.memberList
                                    if player.tag
                                    not in set(p.tag for p in clan_member_list)
                                ]
                                new_leaves = [
                                    player
                                    for player in clan_member_list
                                    if player.tag
                                    not in set(p.tag for p in clan.memberList)
                                ]
                                for join in new_joins:
                                    join_leave_changes.append(
                                        InsertOne(
                                            {
                                                'type': 'join',
                                                'clan': clan.tag,
                                ...
                                members.append(
                                    {
                                        ...
                                        'builderTrophies': member.builderBaseTrophies,
                                ...
                                UpdateOne(
                                    {'tag': clan.tag},
                                    {
                                        '$set': {
                                            ...
                                            f'changes.clanCapital.{raid_week}': {
                    ...
                    if changes:
                        await db_client.global_clans.bulk_write(
