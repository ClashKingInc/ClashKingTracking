...
EMAILS = [
    'apiclashofclans+test44@gmail.com',
...
class AbstractClient:
    ...
    async def __yield_client(self) -> AsyncGenerator[coc.Client, None]:
        ...
        async with coc.Client(
            loop=asyncio.get_event_loop_policy().get_event_loop(),
            key_count=10,
            key_names='DiscordBot',
            throttle_limit=25,
            ...
            if os.getenv('IS_BETA') != 'yes':
                ...
                await client.login(
                    os.getenv('COC_EMAIL'), os.getenv('COC_PASSWORD')
                )  # be aware that hard coding credentials is bad practice!
            while True:
                try:
                    yield client
    ...
    async def get_client(self) -> Client:
        ...
        if not hasattr(self, '__async_gen') and not hasattr(
            self, '_AbstractClient__async_gen'
        ):
            self.__async_gen = (
                self.__yield_client()
            )  # create async generator if needed
