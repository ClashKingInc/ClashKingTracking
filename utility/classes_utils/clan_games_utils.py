...
def get_time_until_next_clan_games():
    ...
    try:
        ...
        next_clan_games_start = now.start_of('month').set(day=22, hour=7, minute=0, second=0)
    except ValueError:
        # If the 22nd doesn't exist (e.g., February in a leap year), move to the next month
        next_clan_games_start = now.add(months=1).start_of('month').set(day=22, hour=7, minute=0, second=0)
