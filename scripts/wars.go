...
const (
	...
	cwlFinalizationDelay         = 30 * time.Minute
	cwlFinalizationRetry         = 5 * time.Minute
...
type warStore interface {
	LoadCWLWarTags(context.Context, string) ([]string, error)
	...
	Reschedule(context.Context, string, time.Time, string, string) error
...
func (s *timescaleWarTargetSource) NextCWLDiscoveryTargetBatch(ctx context.Context, limit int) ([]models.BasicClanRow, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	targets, cursor, err := s.nextCWLBatch(ctx, cwlDiscoveryTargetsSQL, limit, s.cwlDiscoveryCursor)
...
func (s *timescaleWarTargetSource) NextCWLRefreshTargetBatch(ctx context.Context, limit int) ([]models.BasicClanRow, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	targets, cursor, err := s.nextCWLBatch(ctx, cwlRefreshTargetsSQL, limit, s.cwlRefreshCursor)
	s.cwlRefreshCursor = cursor
	return targets, err
}

// CWL pages never wrap within a pass. An empty page marks completion and resets
// the cursor for the next sweep, including when the final page is exactly full.
func (s *timescaleWarTargetSource) nextCWLBatch(ctx context.Context, query string, limit int, cursor string) ([]models.BasicClanRow, string, error) {
	rows, err := s.pool.Query(ctx, query, cursor, max(1, limit))
	if err != nil {
		return nil, cursor, err
	}
	defer rows.Close()
	var out []models.BasicClanRow
	for rows.Next() {
	...
	return out, out[len(out)-1].Tag, nil
}
...
func (s *memoryWarTargetSource) NextCWLDiscoveryTargetBatch(_ context.Context, limit int) ([]models.BasicClanRow, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.cwlCursor >= len(s.targets) {
		s.cwlCursor = 0
		return nil, nil
	}
	end := min(len(s.targets), s.cwlCursor+max(1, limit))
	out := append([]models.BasicClanRow(nil), s.targets[s.cwlCursor:end]...)
	s.cwlCursor = end
	return out, nil
}
...
func (d *warsDomain) do(ctx context.Context, app *platform.App, limiter *clashy.Limiter, req warFetchRequest) (models.WarIngest, error) {
	war, sourceClanTag, err := d.fetchWarForRequest(ctx, app, limiter, req)
	if err != nil {
	...
	if req.StoreOnly && war.State != clashy.WarStateEnded {
		...
		if sourceClanTag == req.OpponentTag {
			opponentClanTag = req.ClanTag
		}
		retryAfter := time.Duration(war.RetryAfter()) * time.Second
		if req.WarTag != "" {
			retryAfter = cwlFinalizationRetry
		}
		return models.WarIngest{}, &scheduledWarPendingError{
			warID: req.WarID, state: war.State,
			retryAfter:       retryAfter,
			preferredClanTag: sourceClanTag, opponentClanTag: opponentClanTag,
		}
	}
	if sourceClanTag == "" {
		sourceClanTag = req.ClanTag
	}
	ingest, err := buildWarIngest(*war, sourceClanTag, req.StoreOnly, req.WarTag, req.ScheduleKey, req.WarID)
	if err != nil {
		return models.WarIngest{}, err
	}
...
func (d *warsDomain) fetchWarForRequest(ctx context.Context, app *platform.App, limiter *clashy.Limiter, req warFetchRequest) (*clashy.ClanWar, string, error) {
	...
	if req.WarTag != "" {
		...
		if err != nil || war == nil || !req.StoreOnly {
			return war, req.ClanTag, err
		}
		if !scheduledWarIsUsable(req, *war) {
			return nil, "", fmt.Errorf("tagged CWL response does not match schedule %s", req.ScheduleKey)
		}
		return war, req.ClanTag, nil
	}
	if req.StoreOnly && req.InitialFinalization && req.OpponentTag != "" && req.OpponentTag != req.ClanTag {
		return d.fetchInitialRegularWar(ctx, app, limiter, req, statsName)
	}
	...
	for _, clanTag := range clanTags {
		...
		if !req.StoreOnly {
			return war, clanTag, nil
		}
		if war != nil && scheduledWarIsUsable(req, *war) {
			return war, clanTag, nil
		}
...
type regularWarPerspective struct {
	tag string
	war *clashy.ClanWar
	err error
}

func (d *warsDomain) fetchInitialRegularWar(ctx context.Context, app *platform.App, limiter *clashy.Limiter, req warFetchRequest, statsName string) (*clashy.ClanWar, string, error) {
	tags := []string{req.ClanTag, req.OpponentTag}
	results := make(chan regularWarPerspective, len(tags))
	for _, tag := range tags {
		tag := tag
		go func() {
			war, err := d.fetchOneWarWithStats(ctx, app, limiter, tag, false, statsName)
			results <- regularWarPerspective{tag: tag, war: war, err: err}
		}()
	}
	perspectives := make([]regularWarPerspective, 0, len(tags))
	for range tags {
		perspectives = append(perspectives, <-results)
	}
	// Keep the source clan as the deterministic tie-breaker even though the two
	// HTTP requests complete in an arbitrary order.
	sort.SliceStable(perspectives, func(i, j int) bool {
		return perspectives[i].tag == req.ClanTag && perspectives[j].tag != req.ClanTag
	})
	for _, perspective := range perspectives {
		if perspective.err == nil && perspective.war != nil && scheduledWarIsUsable(req, *perspective.war) && perspective.war.State == clashy.WarStateEnded {
			return perspective.war, perspective.tag, nil
		}
	}
	var pending *regularWarPerspective
	for i := range perspectives {
		perspective := &perspectives[i]
		if perspective.err != nil || perspective.war == nil || !scheduledWarIsUsable(req, *perspective.war) {
			continue
		}
		if pending == nil || effectiveWarRetryAfter(perspective.war) < effectiveWarRetryAfter(pending.war) {
			pending = perspective
		}
	}
	if pending != nil {
		return pending.war, pending.tag, nil
	}
	for _, perspective := range perspectives {
		if perspective.err != nil && !isSkippableWarFetchError(perspective.err) {
			return nil, "", perspective.err
		}
	}
	return nil, "", errScheduledWarUnavailable
}

func effectiveWarRetryAfter(war *clashy.ClanWar) time.Duration {
	if war == nil || war.RetryAfter() <= 0 {
		return warFinalizationFallbackRetry
	}
	return time.Duration(war.RetryAfter()) * time.Second
}
...
func scheduledWarIsUsable(req warFetchRequest, war clashy.ClanWar) bool {
	if !scheduledWarMatches(req, war) {
		return false
	}
	return war.State == clashy.WarStatePreparation || war.State == clashy.WarStateInWar || war.State == clashy.WarStateEnded
}
...
func buildWarIngest(war clashy.ClanWar, sourceClanTag string, finished bool, warTag, scheduleKey string, warID int32) (models.WarIngest, error) {
	...
	if !finished {
		...
		nextRunAt := warScheduleNextRunAt(endAt, warTag, time.Now().UTC())
...
func warScheduleNextRunAt(endAt time.Time, warTag string, now time.Time) time.Time {
	if warTag != "" && endAt.After(now) {
		return endAt.Add(cwlFinalizationDelay)
	}
	return endAt
}
...
func (d *warsDomain) runCWLLoop(ctx context.Context, app *platform.App, groupLimiter, warLimiter *clashy.Limiter) {
	...
	go func() {
		defer wg.Done()
		d.runCWLSweeps(ctx, app, groupLimiter, warLimiter, false)
	}()
	d.runCWLSweeps(ctx, app, groupLimiter, warLimiter, true)
	wg.Wait()
}
...
func (d *warsDomain) syncCWLSweep(ctx context.Context, app *platform.App, groupLimiter, warLimiter *clashy.Limiter, refresh bool) error {
	limit := max(1, app.Config.CWLRequestsPerSecond*app.Config.TargetPageMultiplier)
	for ctx.Err() == nil {
		var targets []models.BasicClanRow
		var err error
		if refresh {
			targets, err = d.targets.NextCWLRefreshTargetBatch(ctx, limit)
		} else {
			targets, err = d.targets.NextCWLDiscoveryTargetBatch(ctx, limit)
		}
		if err != nil {
			return err
		}
		...
		if err := d.syncCWLTargets(ctx, app, groupLimiter, warLimiter, targets, false); err != nil {
			return err
		}
	}
	return ctx.Err()
...
func (d *warsDomain) syncCWLTargets(ctx context.Context, app *platform.App, groupLimiter, warLimiter *clashy.Limiter, targets []models.BasicClanRow, strict bool) error {
	...
	return runBounded(ctx, platform.RequestConcurrency(app.Config.CWLRequestsPerSecond), targets, func(workerCtx context.Context, target models.BasicClanRow) error {
		...
		previous, err := d.store.LoadCWLWarTags(workerCtx, cwlID)
		if err != nil {
			return err
		}
...
func (d *warsDomain) scheduleCWLWars(ctx context.Context, app *platform.App, limiter *clashy.Limiter, group *clashy.ClanWarLeagueGroup, strict bool) (int, error) {
	...
	for _, warTag := range tags {
		...
		if len(ingest.Schedules) == 0 {
			return 0, errors.New("CWL war response is missing scheduling fields")
		}
		if wars[0].State == clashy.WarStateEnded {
			// The store allocates the real ID from the schedule in the same
			// transaction and rewrites this placeholder before persisting.
			finished, err := buildWarIngest(wars[0], source, true, warTag, ingest.Schedules[0].ScheduleKey, 1)
			if err != nil {
				return 0, err
			}
			finished.Schedules = ingest.Schedules
			ingest = finished
		}
