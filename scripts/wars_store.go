...
func (s *timescaleWarStore) ResolveCWLGroupLeague(ctx context.Context, clanTags []string, fallback int) (int, error) {
	var leagueID int
	err := s.pool.QueryRow(ctx, `
		...
		ORDER BY count(*) DESC, cwl_league_id
		LIMIT 1
	`, clanTags, unrankedWarLeagueID).Scan(&leagueID)
	if errors.Is(err, pgx.ErrNoRows) {
		return fallback, nil
	}
	if err != nil {
		return 0, warStoreError("resolve CWL group league", err)
	}
	return leagueID, nil
}

func (s *timescaleWarStore) LoadStoredCWLGroupLeague(ctx context.Context, cwlID string) (int, bool, error) {
	var leagueID int
	err := s.pool.QueryRow(ctx, `SELECT cwl_league_id FROM cwl_groups WHERE cwl_id = $1 AND cwl_league_id IS NOT NULL`, cwlID).Scan(&leagueID)
	if errors.Is(err, pgx.ErrNoRows) {
		return 0, false, nil
	}
	if err != nil {
		return 0, false, warStoreError("load stored CWL group league", err)
	}
...
const upsertPlayerTimersSQL = `
	...
	WHERE player_timers.expires_at IS DISTINCT FROM EXCLUDED.expires_at
`
...
func (s *memoryWarStore) ResolveCWLGroupLeague(_ context.Context, clanTags []string, fallback int) (int, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	wanted := make(map[string]struct{}, len(clanTags))
	for _, tag := range clanTags {
		wanted[tag] = struct{}{}
	}
	counts := make(map[int]int)
	for _, target := range s.targets {
		if _, ok := wanted[target.Tag]; ok && target.CWLLeagueID != 0 && target.CWLLeagueID != unrankedWarLeagueID {
			counts[target.CWLLeagueID]++
		}
	}
	resolved, bestCount := fallback, 0
	for leagueID, count := range counts {
		if count > bestCount || count == bestCount && leagueID < resolved {
			resolved, bestCount = leagueID, count
		}
	}
	return resolved, nil
}

func (s *memoryWarStore) LoadStoredCWLGroupLeague(_ context.Context, cwlID string) (int, bool, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	row, ok := s.cwlGroups[cwlID]
	if !ok || row.CWLLeagueID == nil {
		return 0, false, nil
	}
	return *row.CWLLeagueID, true, nil
}
...
func (s *memoryWarStore) Store(_ context.Context, ingest models.WarIngest) error {
	...
	for _, row := range ingest.CWLGroups {
		if existing, ok := s.cwlGroups[row.CWLID]; ok {
			if row.CWLLeagueID == nil {
				row.CWLLeagueID = existing.CWLLeagueID
			}
			if row.WarSize == nil {
				row.WarSize = existing.WarSize
			}
		}
	...
	if ingest.FinishedWarID > 0 {
		...
		for key, timer := range s.playerTimers {
			if timer.EventType == "war" && timer.EventKey == ingest.FinishedScheduleKey {
				delete(s.playerTimers, key)
			}
		}
	}
